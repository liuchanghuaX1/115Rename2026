...
// @version         1.3.0
// @description     115网盘视频整理：保留本地标题加工与网络改名分离。支持T28/单字母番号(h637)、防误伤透视引擎(修复DASS截断)、part1转-1分段、彻底免疫空格日期干扰、根治01前缀与重复番号。已知问题：暂不支持JavDB刮削与FC2网站改名。
// @author          sonarlee
...
(function () {
    ...
    function matchCodeByPrefixLibrary(str) {
        ...
        // 1. 单词边界匹配优先 (应对标准的 DASS-234，防止被透视切断)
    ...
    function parseVideoInfo(origTitle) {
        ...
        if (fc2Match && fc2Match[1]) {
            ...
            // Tokyo Hot 处理 (严格支持 N, H, K 单字母 + 4位数字)
        ...
        // 6. 生成纯净的本地剩余标题 (彻底剥离番号、01前缀与所有乱七八糟的括号)
        ...
        return {
            ...
            date: dateStr, // 从原名中剥离出来的本地日期
            localTitle: localTitle // 剩余的最纯净的本地标题
        };
    }
    ...
    // 【本地改名专用】只保留番号、标记与日期，外加洗净的本地标题
    function local_rename(fid, vInfo, suffix, addDate, callback) {
        let newName = buildNewNameUnified(vInfo, vInfo.localTitle, [], addDate ? vInfo.date : "", suffix);
        send_115(fid, newName, vInfo.fullCode, callback);
    }
    
    // 【网络改名专用】只用刮削来的影片名和女优名，彻底抛弃本地标题
    ...
    function rename_multi(fid, vInfo, suffix, addDate, callback) {
        if (/^FC2-PPV-\d{5,7}$/i.test(vInfo.baseCode)) {
            // 已知问题：暂不支持FC2单独刮削改名，仅显示提示
            showPageNotification(`已知问题：暂不支持FC2网站改名`, 'error', 3000);
    ...
    function requestMultiSource(fid, vInfo, suffix, addDate, callback, source) {
        if (source === "javbus") {
            requestJavbus(fid, vInfo, suffix, addDate, javbusDirectAccess, function() {
                ...
                showPageNotification(`无法在JavBus找到"${vInfo.queryCode}"的信息，已知问题：暂不支持JavDB查询`, 'error', 3000);
                if (typeof callback === 'function') callback();
            });
        }
    }

    // 净化网络刮削到的标题 (彻底剔除 "顯示原標題" 和重复的番号)
