...
专为 115 网盘设计的视频整理与重命名脚本，本地加工与多站刮削并行，支持分段统一、智能标记、广告清理、评分同步及归档整理。

## ✨ 功能

### 🧹 本地番号加工
- 番号提取（`T28`、`S2M` 等混合厂牌；`h637` → `TokyoHot-h637`）
- 分段统一（`part1`、`.Part.2`、`(CD3)` → `-1`、`-2`、`-A`）
- 标记识别：`4K`/`8K`/`60fps`/`120fps`/`VR`/`中文字幕`/`无码`/`流出`
- 广告残留清理：自动移除 `[3Q]`、`(原)` 等垃圾标牌

### 🌐 多站改名
- 四站轮询：**JavLibrary → JavBus → xslist → JavDB**
